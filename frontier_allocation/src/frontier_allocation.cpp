...
std::string map_frame, base_frame, goal_topic, pf_stat_topic, cmd_vel_topic, move_base_status, frontier_topic, other_goal;
geometry_msgs::Point frontier, old_frontier,second_nearest_frontier; // Marin ADIL
...
geometry_msgs::PoseStamped other_robot_goal;
...
void Other_goal_Callback(const geometry_msgs::PoseStamped msg) // Marin ADIL
{
 other_robot_goal = msg;
}
...
bool find_next_frontier()
{
    ...
    for (int i = 0; i < global_frontiers.size(); i++)
    {
        ...
        {  
            ...
            {   second_nearest_frontier = frontier; //Marin ADIL
                frontier = global_frontiers[i];
                mindist = dist;
                allocated = true;
            }
    ...
    if (sqrt(pow((other_robot_goal.pose.position.x - frontier.x),2) +pow((other_robot_goal.pose.position.y - frontier.y),2)) < 0.5) // Marin ADIL
        {

           if (((second_nearest_frontier.x!=0) || (second_nearest_frontier.y!=0)) && (global_frontiers.size() > 1 )){
            frontier = second_nearest_frontier;
            ROS_INFO("go to the second nearest frontier ");
           }
        }
    return allocated;
}
...
int main(int argc, char **argv)
{
    ...
    private_nh.param<std::string>("goal_topic_other", other_goal,"/move_base_simple/goal"); // Marin ADIL
    ...
    ros::Subscriber sub = private_nh.subscribe(other_goal, 1, &Other_goal_Callback); // Marin ADIL
    ...
    while (ros::ok())
    {
        ...
        {   move_base_msgs::MoveBaseGoal goal_s; // add by My self
            ...
            goal_s.target_pose.header.frame_id = map_frame;
            goal_s.target_pose.header.stamp = ros::Time::now();
            goal_s.target_pose.pose.position.x = frontier.x;
            goal_s.target_pose.pose.position.y = frontier.y;
            goal_s.target_pose.pose.orientation.x = 0;
            goal_s.target_pose.pose.orientation.y = 0;
            goal_s.target_pose.pose.orientation.z = 0;
            goal_s.target_pose.pose.orientation.w = 1;
            ac.sendGoal(goal_s);
